                ...
                <form action="{{ route('users.update', $user->id) }}" method="POST" x-data="{ role: '{{ old('role', $user->role) }}' }">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <input type="text" id="username" name="username" value="{{ old('username', $user->username) }}" class="w-full border-gray-300 rounded-xl px-4 py-2.5 text-stone-700 focus:border-amber-500 focus:ring-amber-500 shadow-sm" required style="border-radius: 12px; border-color: #e0e0e0;">
                            @error('username')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div class="md:col-span-2"> <label for="email" class="block text-sm font-bold mb-2 text-stone-800">Email</label>
                        ...
                        <div>
                            ...
                            <select name="divisi_id" id="divisi_id" class="w-full border-gray-300 rounded-xl px-4 py-2.5 text-stone-700 focus:border-amber-500 focus:ring-amber-500 shadow-sm" style="border-radius: 12px; border-color: #e0e0e0;">
                                ...
                                @foreach($divisis as $d)
                                    @php
                                        // Cek apakah divisi sudah terisi oleh orang lain
                                        $isOccupiedByOther = $d->ketua_divisi_id && $d->ketua_divisi_id != $user->id;
                                        $isOccupiedByOtherString = $isOccupiedByOther ? 'true' : 'false';
                                        $isCurrentValueString = old('divisi_id', $user->divisi_id) == $d->id ? 'true' : 'false';
                                    @endphp

                                    <option value="{{ $d->id }}" 
                                            {{ $isCurrentValueString === 'true' ? 'selected' : '' }}
                                            
                                            {{-- Sembunyikan jika role di set Ketua Divisi DAN divisi sudah terisi oleh orang lain --}}
                                            x-bind:hidden="role === 'ketua_divisi' && {{ $isOccupiedByOtherString }}"
                                            >
                                        {{ $d->nama }}
